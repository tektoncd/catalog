...
The following task is used to provide static analysis on YAML files mounted using `tslint` (TypeScript linter).

## Installing the Task

```bash
kubectl apply -f https://raw.githubusercontent.com/tektoncd/catalog/master/task/ts-lint/0.1/ts-lint.yaml
```
...
4. Create the Pipeline and PipelineRun for `TypeScript` linter
  ...
  tasks:
    - name: fetch-repository
      ...
      params:
        ...
        - name: deleteExisting
          value: "true"
    - name: ts-lint-run #lint TypeScript
      taskRef:
        name: ts-lint
      runAfter:
        - fetch-repository
